...
[最終提出ページへ](https://recruit-litmus.jp/r/submit?token=42bc50d9-cb9d-4c50-80cd-d177e9c2a781)
...
`CE_AWS` | クラウドエンジニア（AWS）<br/>クラウドインフラの設計、構築、運用が得意な方向け。<br/>Keyword：セキュリティ,ネットワーク,可用性,サーバー,コンテナ,ストレージ,データベース | [`Q-AWSAvailability-Basic`](Q-AWSAvailability-Basic)<br/>[`Q-AWSCloudGovernance-Basic`](Q-AWSCloudGovernance-Basic)<br/>[`Q-AWSKnowledge-Basic`](Q-AWSKnowledge-Basic)<br/>[`Q-AWSNetworkKnowledge-Basic`](Q-AWSNetworkKnowledge-Basic)<br/>[`Q-AWSSecurity-Basic`](Q-AWSSecurity-Basic)
`AE` | アナリティクスエンジニア<br/>データマート作成などのためのデータ集計・変換処理が得意な方向け。<br/>Keyword：データマネジメント,データ抽出,データ加工,データモデリング | [`Q-DataSummarization-Basic`](Q-DataSummarization-Basic)
...
* `フィードバック` : `提出` 後にこのリポジトリに作成されるissueに対し、通常10分-1時間ほどで追加されるコメントのこと
  * `フィードバック` は、 `https://github.com/recruit-skillcheck/exam_142f01ff5c/issues` から確認できます。必要に応じて確認し解答を修正してください
...
* `最終提出` : その時点で最終の `提出` を、スキルチェックの最終的な`提出物`として確定させること
  * `方法`: [こちらのリンク](https://recruit-litmus.jp/r/submit?token=42bc50d9-cb9d-4c50-80cd-d177e9c2a781)から最終提出ページに遷移し、「提出する」ボタンを押していただくと完了します
...
<details><summary>Rライブラリ</summary>
